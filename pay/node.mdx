...
<Note>
AiFinPay is **token-free**. Agents pay in real assets — USDC, USDT, SOL, MATIC — settled on Solana and Polygon mainnet. There is no AiFinPay token to buy or hold. There is no internal credit to earn either: mSECCO was retired on 2026-08-06 and was never minted — crediting it cost an extra chain call on every payment.
</Note>
...
<Note>
The snapshot may still report `msecco_balance` per chain. It is always zero: mSECCO was retired on 2026-08-06, and the token was never minted. Ignore the field; it will be removed.
</Note>
