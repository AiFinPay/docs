...
description: "Agent identity and dashboard linking in Python with aifinpay-agent. Python does not pay AIFP-1 merchants yet — use MCP or Node for payments."
---
...
token-free — there is nothing to buy, stake, or trade. The internal credit
that used to be earned on paid calls was retired on 2026-08-06 and was never
minted, so there is no accounting unit to hold either.
</Note>
...
(`fetchPaid`) and the MCP server (`payable_fetch`). Use one of those to pay.
`agent.sign_dashboard_claim(challenge)` links a Python agent to its owner's
dashboard (My Agents → Add agent by address).
</Warning>
...
## The retired Seat flow

The Solana Seat invoice flow (`reserve_seat_invoice`) is retired: the Solana
program is closed and cannot execute. Do not pay it.
