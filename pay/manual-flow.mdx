...
<Steps>
  ...
  <Step title="Read the 402 challenge">
    ...
    ```json
    {
      ...
      "protocol": "AIFP-1 (contracts v5.3)",
    ...
    <Note>
    In the `protocol` field, **AIFP-1** is the payment protocol version and **v5.3** is the revision of the AiFinPayCore contract on Polygon — two independent version lines. SDKs should key behavior off `AIFP-1`, never off the contract revision.
    </Note>
  </Step>
...
<Steps>
  <Step title="Request with no headers → 402 + a fresh nonce">
    ...
    ```json
    {
      ...
      "min_usd": 1,
      "min_msecco": 100,          // vestigial — mSECCO was retired 2026-08-06; ignore it
      "idl_uri": "/idl.json",
      "usdc_mint": "…",
      "usdt_mint": "…",
      "usdc_ata_treasury": "…",
