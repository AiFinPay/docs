...
no code at all; [self-hosted verification](/charge/dashboard) is the alternative if you
...
AIFP-1 settles one quoted **gross** amount through a verified B2BSplitter v1.3
route:

```text
payer total = quoted gross
                     ├──── 99% ────▶ merchant wallet
                     ├────  1% ────▶ AiFinPay treasury
                     └────  0% ────▶ creator/referral
```

The fee is not added on top. AIFP-2/x402 is a separate route with a current 0%
AiFinPay protocol fee; the provider receives 100% and network gas is separate.

A contract address or historical transaction is not sufficient readiness
evidence. The bridge must remain fail-closed until its exact v1.3 address,
runtime hash, on-chain BPS profile, merchant target, asset metadata and paid E2E
evidence are pinned for the release. Historical v1.1/v1.2
`payMatic`/three-way splitters are legacy and must not be selected.
