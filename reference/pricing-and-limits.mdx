...
[self-hosted verification](/charge/dashboard). If you are an agent *paying* for
services, you want [Pay for services](/pay/node) instead; that is a different
protocol with different headers.

## Units

Everything is metered in **billing units**. One unit is **$0.0005**, so 2,000
units is $1.
...
you about **$0.0005** — half a cent. A million paid requests is roughly
$500. If your goal is revenue, this pays off at API-scale volume. If your goal is
...
- **Hosted Gateway** — give them your gateway base URL:
  ...
  not routing.
- **Self-hosted verification** — give them your own domain, exactly as you
  would without AiFinPay. There is no AiFinPay URL in the path; we only issue the
  quote and the receipt, and your server verifies it inline (a few lines on a
  standard JWT library — `jose` in Node, `pyjwt` in Python — against our JWKS;
  there is no AiFinPay merchant SDK to install).
