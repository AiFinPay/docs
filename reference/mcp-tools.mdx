...
description: "The 7 tools exposed by @aifinpay/mcp — the agent payment + identity surface. (A read-only directory toolset is on the roadmap.)"
---
...
and there is no token to buy. The internal credit that used to be earned was retired on 2026-08-06 and never minted, so there is no
tradable asset.
...
| `pay_with_split` | `(legacy fields accepted)` | Retired compatibility tool. Returns `legacy_split_route_retired`; never creates an invoice or moves funds. |
| `quote_split` | `(legacy fields accepted)` | Retired compatibility tool. Use a canonical AIFP-1 quote. |
...
| `quote_cost` | `({ amount, asset? })` | Preview gross-inclusive AIFP-1 economics (99/1/0) without paying. |
